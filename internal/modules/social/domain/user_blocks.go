...
type User_Blocks struct {
	...
	// Người chặn
	BlockerUser *domain.User `gorm:"foreignKey:Blocker_UserID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
	// Người bị chặn
	BlockedUser *domain.User `gorm:"foreignKey:Blocked_UserID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
}
