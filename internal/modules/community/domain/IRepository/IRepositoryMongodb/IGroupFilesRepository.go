...
type IGroupFilesRepository interface {
	...
	CreateBulkGroupFiles(ctx context.Context, files []entity.GroupFile) (int64, []*mongodbErrors.BulkError, error)
	...
	DeleteBulkGroupFilesByGroupID(ctx context.Context, groupIDs []string) (int64, []*mongodbErrors.BulkError, error)
	DeleteGroupFileByIDAndGroupID(ctx context.Context, id string, groupID string) error
	DeleteBulkGroupFileByIDAndGroupID(ctx context.Context, ids []string, groupID string) (int64, []*mongodbErrors.BulkError, error)
	DeleteGroupFilesByGroupIDAndUploaderID(ctx context.Context, groupID string, uploaderID string) error
	DeleteBulkGroupFilesByGroupIDAndUploaderID(ctx context.Context, groupID string, uploaderIDs []string) (int64, []*mongodbErrors.BulkError, error)
}
