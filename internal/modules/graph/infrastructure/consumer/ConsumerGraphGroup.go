...
type ConsumerGraphGroup struct {
...
func NewConsumerGraphGroup(events events.EventBus, pool IRepositoryShare.IWorkerPool, redisRepo IRepositoryShare.IRedis, graphRepo neo4j.IGraphRepository) *ConsumerGraphGroup {
	return &ConsumerGraphGroup{
		events:    events,
		pool:      pool,
		redisRepo: redisRepo,
		graphRepo: graphRepo,
...
func (c *ConsumerGraphGroup) ConsumerGraphGroup(ctx context.Context) error {
	err := c.events.SubscribeBatch(ctx, constants.TopicGraphGroup.String(), 100, time.Duration(5)*time.Minute, func(ctx context.Context, events []events.IntegrationEvent) error {
...
func (c *ConsumerGraphGroup) handleCreatedEvent(ctx context.Context, event events.IntegrationEvent) error {
	data, err := utils.ParsePayload[graphEvent.GroupNodePayload](event.Payload)
	if err != nil {
	...
	err = c.graphRepo.UpsertGroupNode(ctx, &entity.GroupNode{
		GroupID:     data.GroupID,
		Privacy:     data.Privacy,
		MemberCount: data.MemberCount,
	})
	if err != nil {
		return fmt.Errorf("failed to upsert group node: %w", err)
	}
	return nil
}
func (c *ConsumerGraphGroup) handleUpdatedEvent(ctx context.Context, event events.IntegrationEvent) error {
	data, err := utils.ParsePayload[graphEvent.GroupNodePayload](event.Payload)
	if err != nil {
		return kafka.NewNonRetryableError(fmt.Errorf("failed to parse event payload: %w", err))
	}
...
func (c *ConsumerGraphGroup) handleDeletedEvent(ctx context.Context, event events.IntegrationEvent) error {
	data, err := utils.ParsePayload[graphEvent.DeleteGroupNodePayload](event.Payload)
	if err != nil {
		return kafka.NewNonRetryableError(fmt.Errorf("failed to parse event payload: %w", err))
	}
	if data == nil {
	...
	err = c.graphRepo.DeleteGroupNode(ctx, data.GroupID)
	if err != nil {
		return fmt.Errorf("failed to delete group node: %w", err)
	}
	return nil
}
func (c *ConsumerGraphGroup) ConsumerFailedGraphGroup(ctx context.Context) error {
	return nil
}
