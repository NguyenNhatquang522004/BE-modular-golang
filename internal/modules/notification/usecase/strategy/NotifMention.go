...
func NewNotifMention(notificationRepo IRepositoryCassandra.INotificationsRepository,
	notificationTemplateRepo IRepositoryMongodb.INotificationTemplatesRepository,
	userNotificationSettingsRepo IRepositoryMongodb.IUserNotificationSettingsRepository,
	pool IRepositoryShare.IWorkerPool, socketManager socket.Manager) *NotifMention {
	return &NotifMention{
...
func (r *NotifMention) Execute(ctx context.Context, req *notificationEvent.NotificationPayload) error {
	...
	for _, userID := range req.SendUser {
		err := r.pool.Run(ctx, func() {
			...
			if datauser.Settings.PushEnabled == false || datauser.Settings.PushInteractions == false || datauser.Settings.PushGroups == false  || datauser.Settings.PushFriends == false {
				// User has muted all notifications or interaction notifications, skip sending
				return
			}
...
func (r *NotifMention) GetType() sharedEnums.NotificationType {
