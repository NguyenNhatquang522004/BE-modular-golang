...
func NewNotifPostLike(notificationRepo IRepositoryCassandra.INotificationsRepository,
	notificationTemplateRepo IRepositoryMongodb.INotificationTemplatesRepository,
	userNotificationSettingsRepo IRepositoryMongodb.IUserNotificationSettingsRepository,
	pool IRepositoryShare.IWorkerPool, socketManager socket.Manager) *NotiPostLike {
	return &NotiPostLike{
...
func (r *NotiPostLike) Execute(ctx context.Context, req *notificationEvent.NotificationPayload) error {
	...
	for _, userID := range req.SendUser {
		err := r.pool.Run(ctx, func() {
			...
			if datauser.Settings.PushEnabled == false || datauser.Settings.PushFriends == false || datauser.Settings.PushGroups == false || datauser.Settings.PushInteractions == false {
			...
			var entityNotification = &entity.Notification{
				...
				Type:           sharedEnums.NotifPostLike,
				ActorID:        ActorID,
...
func (r *NotiPostLike) GetType() sharedEnums.NotificationType {
