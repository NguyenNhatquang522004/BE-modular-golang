...
func NewNotifFriendAccept(notificationRepo IRepositoryCassandra.INotificationsRepository,
	notificationTemplateRepo IRepositoryMongodb.INotificationTemplatesRepository,
	userNotificationSettingsRepo IRepositoryMongodb.IUserNotificationSettingsRepository,
	pool IRepositoryShare.IWorkerPool, socketManager socket.Manager) *NotifFriendAccept {
...
func (r *NotifFriendAccept) Execute(ctx context.Context, req *notificationEvent.NotificationPayload) error {
	...
	for _, userID := range req.SendUser {
		err := r.pool.Run(ctx, func() {
			...
			if datauser.Settings.PushEnabled == false || datauser.Settings.PushFriends == false {
				// User has muted all notifications or friend accept notifications, skip sending
				return
			}
			...
			var entityNotification = &entity.Notification{
				...
				Type:           sharedEnums.NotifFriendAccept,
				ActorID:        ActorID,
				ActorName:      dataActor.Name,
				ActorAvatar:    dataActor.Avatar,
				TargetID:       req.FriendAcceptID,
				TargetPreview:  "",
				IsRead:         false,
				IsClicked:      false,
				GroupKey:       ""}
...
func (r *NotifFriendAccept) GetType() sharedEnums.NotificationType {
